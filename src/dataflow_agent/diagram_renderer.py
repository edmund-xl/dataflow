...
NETWORK_NODE_WIDTH = 270
NETWORK_NODE_HEIGHT = 88
NETWORK_SUMMARY_WIDTH = 560
NETWORK_BOUNDARY_TYPES = {"gcp_project", "network", "vpc", "subnet", "nat", "psc_peering", "lb", "entry_point", "server"}
NETWORK_CONTROL_TYPES = {"firewall_rule", "cloud_armor_policy"}
LAYER_ORDER = {
    "project": 0,
    "network": 1,
    "edge": 1,
...
def render_diagrams(graph: GraphModel, diagrams_dir: Path) -> list[Path]:
    ...
    for view in VIEWS:
        ...
        elif view.filename == "01_network_layer":
            nodes, edges, controls, control_edges = _select_network_boundary_view(nodes, edges)
            positions = _network_boundary_layout(nodes, edges)
            outputs.append(_write_network_boundary_svg(diagrams_dir / f"{view.filename}.svg", view, nodes, edges, controls, control_edges, positions))
            outputs.append(_write_network_boundary_png(diagrams_dir / f"{view.filename}.png", view, nodes, edges, controls, control_edges, positions))
            outputs.append(_write_network_boundary_pdf(diagrams_dir / f"{view.filename}.pdf", view, nodes, edges, controls, control_edges, positions))
        else:
        ...
        outputs.extend(
            write_editable_outputs(
                ...
                edges,
                positions,
                node_width=_editable_node_width(view),
                node_height=_editable_node_height(view),
            )
        )
    return outputs
...
def _editable_node_width(view: View) -> int:
    if view.filename in {"00_overview", "03_service_dependency_layer"}:
        return SERVICE_DEP_NODE_WIDTH
    if view.filename == "01_network_layer":
        return NETWORK_NODE_WIDTH
    return NODE_WIDTH


def _editable_node_height(view: View) -> int:
    if view.filename in {"00_overview", "03_service_dependency_layer"}:
        return SERVICE_DEP_NODE_HEIGHT
    if view.filename == "01_network_layer":
        return NETWORK_NODE_HEIGHT
    return NODE_HEIGHT


def _select_network_boundary_view(nodes: list[GraphNode], edges: list[GraphEdge]) -> tuple[list[GraphNode], list[GraphEdge], list[GraphNode], list[GraphEdge]]:
    node_by_id = {node.id: node for node in nodes}
    boundary_nodes = [node for node in nodes if node.type in NETWORK_BOUNDARY_TYPES]
    boundary_ids = {node.id for node in boundary_nodes}
    structure_edges = [
        edge
        for edge in edges
        if edge.type == "contains" and edge.source in boundary_ids and edge.target in boundary_ids and _network_structure_edge_is_displayable(edge, node_by_id)
    ]
    connected_ids = {edge.source for edge in structure_edges}.union(edge.target for edge in structure_edges)
    for node in boundary_nodes:
        if node.type in {"gcp_project", "vpc", "subnet", "nat", "psc_peering", "lb", "entry_point", "server"}:
            connected_ids.add(node.id)
    boundary_nodes = [node for node in boundary_nodes if node.id in connected_ids or node.type in {"gcp_project", "vpc", "subnet"}]
    boundary_ids = {node.id for node in boundary_nodes}
    structure_edges = [edge for edge in structure_edges if edge.source in boundary_ids and edge.target in boundary_ids]
    controls = sorted(
        [node for node in nodes if node.type in NETWORK_CONTROL_TYPES],
        key=lambda node: (_network_control_record_id(node, edges), node.label.lower(), node.id),
    )
    control_ids = {node.id for node in controls}
    control_edges = [edge for edge in edges if edge.target in control_ids and edge.type in {"contains", "allowed_by", "protected_by"}]
    return boundary_nodes or [node_by_id[node_id] for node_id in sorted(boundary_ids) if node_id in node_by_id], structure_edges, controls, control_edges


def _network_boundary_layout(nodes: list[GraphNode], edges: list[GraphEdge]) -> dict[str, tuple[int, int]]:
    columns: dict[int, list[GraphNode]] = {}
    for node in nodes:
        columns.setdefault(_network_node_column(node), []).append(node)
    edge_order: dict[str, str] = {}
    for edge in edges:
        edge_order.setdefault(edge.target, _edge_record_id(edge) or edge.id)
    for col_nodes in columns.values():
        col_nodes.sort(key=lambda node: (edge_order.get(node.id, ""), _network_node_column(node), node.label.lower(), node.id))

    positions: dict[str, tuple[int, int]] = {}
    x_step = NETWORK_NODE_WIDTH + 105
    y_step = NETWORK_NODE_HEIGHT + 42
    for col_idx, col in enumerate(sorted(columns)):
        for row_idx, node in enumerate(columns[col]):
            positions[node.id] = (LEFT_MARGIN + col_idx * x_step, HEADER_HEIGHT + 70 + row_idx * y_step)
    return positions


def _network_node_column(node: GraphNode) -> int:
    return {
        "gcp_project": 0,
        "network": 1,
        "vpc": 1,
...
def _network_structure_edge_is_displayable(edge: GraphEdge, node_by_id: dict[str, GraphNode]) -> bool:
    source = node_by_id.get(edge.source)
    target = node_by_id.get(edge.target)
    if not source or not target:
        return False
    if source.type == "gcp_project" and target.type == "server":
        return False
    return target.type in {"network", "vpc", "subnet", "nat", "psc_peering", "lb", "entry_point", "server"}


def _network_boundary_size(positions: dict[str, tuple[int, int]], controls: list[GraphNode]) -> tuple[int, int, int]:
    if not positions:
        graph_width = MIN_WIDTH
        graph_height = MIN_HEIGHT
    else:
        graph_width = max(x for x, _ in positions.values()) + NETWORK_NODE_WIDTH + 90
        graph_height = max(y for _, y in positions.values()) + NETWORK_NODE_HEIGHT + 92
    summary_x = graph_width + 40
    width = summary_x + NETWORK_SUMMARY_WIDTH + 50
    summary_height = HEADER_HEIGHT + 120 + max(1, len(controls)) * 70
    height = max(MIN_HEIGHT, graph_height, summary_height)
    return width, height, summary_x


def _write_network_boundary_svg(path: Path, view: View, nodes: list[GraphNode], edges: list[GraphEdge], controls: list[GraphNode], control_edges: list[GraphEdge], positions: dict[str, tuple[int, int]]) -> Path:
    width, height, summary_x = _network_boundary_size(positions, controls)
    lines = [
        f'<svg xmlns="http://www.w3.org/2000/svg" width="{width}" height="{height}" viewBox="0 0 {width} {height}" role="img" aria-label="{xml_escape(view.title)} network boundary view">',
        "<defs>",
        '<marker id="networkArrow" markerWidth="10" markerHeight="10" refX="9" refY="5" orient="auto"><path d="M0,0 L10,5 L0,10 Z" fill="#64748B"/></marker>',
        '<filter id="networkShadow"><feDropShadow dx="3" dy="5" stdDeviation="0" flood-color="#CBD5E1" flood-opacity="0.9"/></filter>',
        "</defs>",
        ...
        '<text x="60" y="82" font-family="Arial, Helvetica, sans-serif" font-size="14" fill="#5B6573">Network boundary view: project, VPC/subnet, NAT/PSC, entry and server placement</text>',
        f'<text x="60" y="104" font-family="Arial, Helvetica, sans-serif" font-size="12" fill="#5B6573">{len(nodes)} boundary nodes | {len(edges)} real network structure edges | firewall details summarized</text>',
        f'<rect x="40" y="{HEADER_HEIGHT}" width="{summary_x - 58}" height="{height - HEADER_HEIGHT - 42}" rx="18" fill="#FFFFFF" stroke="#CBD5E1"/>',
        f'<rect x="{summary_x}" y="{HEADER_HEIGHT}" width="{NETWORK_SUMMARY_WIDTH}" height="{height - HEADER_HEIGHT - 42}" rx="18" fill="#FFFFFF" stroke="#CBD5E1"/>',
    ]
    _append_network_svg_edges(lines, edges, positions)
    for node in nodes:
        x, y = positions.get(node.id, (LEFT_MARGIN, HEADER_HEIGHT + 70))
        _append_network_svg_node(lines, node, x, y)
    _append_network_svg_summary(lines, controls, control_edges, summary_x)
    lines.append("</svg>")
    path.write_text("\n".join(lines), encoding="utf-8")
    return path


def _append_network_svg_edges(lines: list[str], edges: list[GraphEdge], positions: dict[str, tuple[int, int]]) -> None:
    for edge in edges:
        points = _network_edge_points(edge, positions)
        if not points:
            continue
        point_text = " ".join(f"{x:.1f},{y:.1f}" for x, y in points)
        lines.append(f'<polyline points="{point_text}" fill="none" stroke="#64748B" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round" marker-end="url(#networkArrow)" data-edge-id="{xml_escape(edge.id)}" data-edge-type="{xml_escape(edge.type)}" data-source="{xml_escape(edge.source)}" data-target="{xml_escape(edge.target)}"><title>{xml_escape(_edge_accessible_label(edge))}</title></polyline>')
        label_x = (points[-2][0] + points[-1][0]) / 2
        label_y = (points[-2][1] + points[-1][1]) / 2 - 8
        lines.append(f'<rect x="{label_x - 35:.1f}" y="{label_y - 12:.1f}" width="70" height="20" rx="10" fill="#FFFFFF" stroke="#CBD5E1" stroke-width="0.8"/>')
        lines.append(f'<text x="{label_x:.1f}" y="{label_y + 3:.1f}" text-anchor="middle" font-family="Arial, Helvetica, sans-serif" font-size="10" font-weight="700" fill="#475569">{xml_escape(edge.type)}</text>')


def _append_network_svg_node(lines: list[str], node: GraphNode, x: int, y: int) -> None:
    style = _node_style(node, dark=False)
    risk_level = _node_risk_level(node)
    lines.append(f'<g filter="url(#networkShadow)" data-node-id="{xml_escape(node.id)}" data-node-type="{xml_escape(node.type)}" role="img" aria-label="{xml_escape(_node_accessible_label(node))}" data-risk-level="{risk_level}"><title>{xml_escape(node.id)} | {xml_escape(node.type)}</title>')
    lines.append(f'<rect x="{x}" y="{y}" width="{NETWORK_NODE_WIDTH}" height="{NETWORK_NODE_HEIGHT}" rx="10" fill="{style.fill}" stroke="{style.stroke}" stroke-width="2"/>')
    lines.append(f'<rect x="{x}" y="{y}" width="8" height="{NETWORK_NODE_HEIGHT}" rx="4" fill="{style.accent}"/>')
    lines.append(f'<text x="{x + 20}" y="{y + 24}" font-family="Arial, Helvetica, sans-serif" font-size="11" font-weight="700" fill="{style.accent}">{xml_escape(_display_type(node.type))}</text>')
    lines.append(f'<text x="{x + 20}" y="{y + 49}" font-family="Arial, Helvetica, sans-serif" font-size="15" font-weight="700" fill="#111827">{xml_escape(_clip(node.label, 30))}</text>')
    detail = _node_detail(node)
    if detail:
        lines.append(f'<text x="{x + 20}" y="{y + 72}" font-family="Arial, Helvetica, sans-serif" font-size="11" fill="#64748B">{xml_escape(_clip(detail, 34))}</text>')
    lines.append("</g>")


def _append_network_svg_summary(lines: list[str], controls: list[GraphNode], edges: list[GraphEdge], x: int) -> None:
    lines.append(f'<text x="{x + 30}" y="{HEADER_HEIGHT + 42}" font-family="Arial, Helvetica, sans-serif" font-size="18" font-weight="700" fill="#111827">Network Controls Summary</text>')
    lines.append(f'<text x="{x + 30}" y="{HEADER_HEIGHT + 68}" font-family="Arial, Helvetica, sans-serif" font-size="12" fill="#5B6573">Firewall and Cloud Armor details stay in reports and security layer.</text>')
    if not controls:
        lines.append(f'<text x="{x + 30}" y="{HEADER_HEIGHT + 110}" font-family="Arial, Helvetica, sans-serif" font-size="13" fill="#64748B">No network controls found.</text>')
        return
    y = HEADER_HEIGHT + 112
    for node in controls:
        color = "#DC2626" if node.type == "firewall_rule" else "#EA580C"
        fill = "#FFF1F2" if node.type == "firewall_rule" else "#FFF7ED"
        record_id = _network_control_record_id(node, edges) or node.id
        badges = _network_control_badges(node)
        lines.append(f'<g data-network-control-id="{xml_escape(node.id)}" data-source-record="{xml_escape(record_id)}">')
        lines.append(f'<rect x="{x + 24}" y="{y - 22}" width="{NETWORK_SUMMARY_WIDTH - 48}" height="56" rx="10" fill="{fill}" stroke="{color}" stroke-width="1.2"/>')
        lines.append(f'<rect x="{x + 24}" y="{y - 22}" width="7" height="56" rx="3.5" fill="{color}"/>')
        lines.append(f'<text x="{x + 44}" y="{y - 3}" font-family="Arial, Helvetica, sans-serif" font-size="13" font-weight="700" fill="#111827">{xml_escape(_clip(record_id + " | " + node.label, 58))}</text>')
        lines.append(f'<text x="{x + 44}" y="{y + 18}" font-family="Arial, Helvetica, sans-serif" font-size="11" fill="#64748B">{xml_escape(_clip(_network_control_detail(node), 64))}</text>')
        badge_x = x + NETWORK_SUMMARY_WIDTH - 42
        for badge in reversed(badges[:3]):
            badge_width = max(42, len(badge) * 7 + 18)
            badge_x -= badge_width
            lines.append(f'<rect x="{badge_x}" y="{y - 14}" width="{badge_width}" height="20" rx="10" fill="#FFFFFF" stroke="{color}" stroke-width="0.9"/>')
            lines.append(f'<text x="{badge_x + badge_width / 2:.1f}" y="{y}" text-anchor="middle" font-family="Arial, Helvetica, sans-serif" font-size="9" font-weight="700" fill="{color}">{xml_escape(badge)}</text>')
            badge_x -= 8
        lines.append("</g>")
        y += 70


def _write_network_boundary_png(path: Path, view: View, nodes: list[GraphNode], edges: list[GraphEdge], controls: list[GraphNode], control_edges: list[GraphEdge], positions: dict[str, tuple[int, int]]) -> Path:
    width, height, summary_x = _network_boundary_size(positions, controls)
    ...
    note_font = _font(14)
    small_font = _font(12)
    node_title_font = _font(15, bold=True)
    node_type_font = _font(11, bold=True)
    draw.text((60, 25), view.title, fill="#111827", font=title_font)
    draw.text((60, 68), "Network boundary view: project, VPC/subnet, NAT/PSC, entry and server placement", fill="#5B6573", font=note_font)
    draw.text((60, 94), f"{len(nodes)} boundary nodes | {len(edges)} real network structure edges | firewall details summarized", fill="#5B6573", font=small_font)
    draw.rounded_rectangle((40, HEADER_HEIGHT, summary_x - 18, height - 42), radius=18, fill="#FFFFFF", outline="#CBD5E1")
    draw.rounded_rectangle((summary_x, HEADER_HEIGHT, summary_x + NETWORK_SUMMARY_WIDTH, height - 42), radius=18, fill="#FFFFFF", outline="#CBD5E1")
    for edge in edges:
        points = _network_edge_points(edge, positions)
        if not points:
            continue
        _draw_polyline(draw, points, "#64748B", width=3, dashed=False)
        _draw_arrowhead(draw, points[-2], points[-1], "#64748B")
        label_x = (points[-2][0] + points[-1][0]) / 2
        label_y = (points[-2][1] + points[-1][1]) / 2 - 8
        draw.rounded_rectangle((label_x - 35, label_y - 12, label_x + 35, label_y + 8), radius=10, fill="#FFFFFF", outline="#CBD5E1")
        draw.text((label_x - _text_width(draw, edge.type, small_font) / 2, label_y - 8), edge.type, fill="#475569", font=small_font)
    for node in nodes:
        x, y = positions.get(node.id, (LEFT_MARGIN, HEADER_HEIGHT + 70))
        style = _node_style(node, dark=False)
        draw.rounded_rectangle((x + 3, y + 5, x + NETWORK_NODE_WIDTH + 3, y + NETWORK_NODE_HEIGHT + 5), radius=10, fill="#CBD5E1")
        draw.rounded_rectangle((x, y, x + NETWORK_NODE_WIDTH, y + NETWORK_NODE_HEIGHT), radius=10, fill=style.fill, outline=style.stroke, width=2)
        draw.rounded_rectangle((x, y, x + 8, y + NETWORK_NODE_HEIGHT), radius=4, fill=style.accent)
        draw.text((x + 20, y + 12), _display_type(node.type), fill=style.accent, font=node_type_font)
        draw.text((x + 20, y + 35), _clip(node.label, 30), fill="#111827", font=node_title_font)
        detail = _node_detail(node)
        if detail:
            draw.text((x + 20, y + 62), _clip(detail, 34), fill="#64748B", font=_font(11))
    draw.text((summary_x + 30, HEADER_HEIGHT + 25), "Network Controls Summary", fill="#111827", font=_font(18, bold=True))
    draw.text((summary_x + 30, HEADER_HEIGHT + 56), "Firewall and Cloud Armor details stay in reports and security layer.", fill="#5B6573", font=small_font)
    y = HEADER_HEIGHT + 90
    for node in controls:
        color = "#DC2626" if node.type == "firewall_rule" else "#EA580C"
        fill = "#FFF1F2" if node.type == "firewall_rule" else "#FFF7ED"
        record_id = _network_control_record_id(node, control_edges) or node.id
        draw.rounded_rectangle((summary_x + 24, y, summary_x + NETWORK_SUMMARY_WIDTH - 24, y + 56), radius=10, fill=fill, outline=color, width=1)
        draw.rounded_rectangle((summary_x + 24, y, summary_x + 31, y + 56), radius=4, fill=color)
        draw.text((summary_x + 44, y + 12), _clip(record_id + " | " + node.label, 58), fill="#111827", font=_font(13, bold=True))
        draw.text((summary_x + 44, y + 35), _clip(_network_control_detail(node), 64), fill="#64748B", font=_font(11))
        badge_x = summary_x + NETWORK_SUMMARY_WIDTH - 42
        for badge in reversed(_network_control_badges(node)[:3]):
            badge_width = max(42, len(badge) * 7 + 18)
            badge_x -= badge_width
            draw.rounded_rectangle((badge_x, y + 8, badge_x + badge_width, y + 28), radius=10, fill="#FFFFFF", outline=color)
            draw.text((badge_x + badge_width / 2 - _text_width(draw, badge, _font(9, bold=True)) / 2, y + 12), badge, fill=color, font=_font(9, bold=True))
            badge_x -= 8
        y += 70
    image.save(path)
    return path
...
def _write_network_boundary_pdf(path: Path, view: View, nodes: list[GraphNode], edges: list[GraphEdge], controls: list[GraphNode], control_edges: list[GraphEdge], positions: dict[str, tuple[int, int]]) -> Path:
    png_path = path.with_suffix(".pdf.png")
    _write_network_boundary_png(png_path, view, nodes, edges, controls, control_edges, positions)
...
def _network_edge_points(edge: GraphEdge, positions: dict[str, tuple[int, int]]) -> list[tuple[float, float]]:
    if edge.source not in positions or edge.target not in positions:
        return []
    source = positions[edge.source]
    target = positions[edge.target]
    sx = source[0] + NETWORK_NODE_WIDTH
    sy = source[1] + NETWORK_NODE_HEIGHT / 2
    tx = target[0]
    ty = target[1] + NETWORK_NODE_HEIGHT / 2
    if sx <= tx:
        mid_x = (sx + tx) / 2
        return [(sx, sy), (mid_x, sy), (mid_x, ty), (tx, ty)]
    gutter = max(source[0] + NETWORK_NODE_WIDTH, target[0] + NETWORK_NODE_WIDTH) + 56
    return [(sx, sy), (gutter, sy), (gutter, ty), (tx, ty)]


def _network_control_record_id(node: GraphNode, edges: list[GraphEdge]) -> str:
    related = [edge for edge in edges if edge.target == node.id]
    record_ids = sorted({_edge_record_id(edge) for edge in related if _edge_record_id(edge)})
    return record_ids[0] if record_ids else ""


def _network_control_detail(node: GraphNode) -> str:
    parts = []
    if node.metadata.get("ports"):
        parts.append(f"ports: {node.metadata['ports']}")
    if node.metadata.get("action"):
        parts.append(f"action: {node.metadata['action']}")
    if node.metadata.get("source"):
        parts.append(f"source: {node.metadata['source']}")
    if node.metadata.get("entry"):
        parts.append(f"entry: {node.metadata['entry']}")
    return " | ".join(parts) or node.type


def _network_control_badges(node: GraphNode) -> list[str]:
    badges: list[str] = []
    if _status_kind(node.status) == "exception":
        badges.append("EXC")
    source = str(node.metadata.get("source", "")).lower()
    ports = str(node.metadata.get("ports", "")).lower()
    label = f"{node.id} {node.label}".lower()
    if "0.0.0.0/0" in source or "::/0" in source:
        badges.append("WIDE")
    if "22" in ports or "ssh" in label:
        badges.append("SSH")
    if node.type == "cloud_armor_policy":
        badges.append("ARMOR")
    return badges
