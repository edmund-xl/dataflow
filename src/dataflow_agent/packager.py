...
def write_package_readme(paths: RuntimePaths, metadata: dict) -> None:
    lines = [
        ...
        *( [f"- 草稿原因：{metadata['draft_reason']}"] if metadata.get("draft_reason") else [] ),
        ...
        "",
        "总览图、服务依赖图和安全/监控图采用“入口上下文 + 主数据流 + 控制摘要 / 覆盖矩阵 + edge ledger”的信息丰富视图：图上只保留真实数据流主线，runtime、Firewall、IAM、Monitoring 作为上下文摘要、节点覆盖标记、Security / Monitoring Ledger 和 Coverage Matrix 展示，而不是画成长穿越数据流线。主数据流优先使用 ELK layered orthogonal layout 排版；若 Node.js / elkjs 不可用，则回退到内置确定性布局。线条使用正交路由和浅色 halo；端口、协议、状态、来源记录、关系类型和源/目标对象以碰撞避让的 inline edge detail 卡片展示，并同步保留在右侧 ledger，避免局部标签和线条堆叠。",
        "",
        "风险和一致性判断不依赖人工看图。`reports/architecture_findings.md` 会直接分析 Excel/DCP 生成的 graph model、校验结果、风险结果和可审查事实，综合列出服务、依赖、数据资产、外部系统、网络、CI/CD、安全、监控、IAM、证据、总览图就绪度、覆盖矩阵、缺失与风险清单、真实数据流链路、问题分组和源表修复位置。`reports/architecture_findings.json` 提供同等内容的机器可读版本。",
        "",
        ...
        *( [f"- Draft reason: {metadata['draft_reason']}"] if metadata.get("draft_reason") else [] ),
        ...
        "",
        "The overview, service dependency, and security / monitoring diagrams use the information-rich entry-context, primary-dataflow, control-summary or coverage-matrix, and edge-ledger view: only real primary dataflow lines stay on the canvas. Runtime, Firewall, IAM, and Monitoring relationships are shown as context summaries, node overlays, the Security / Monitoring Ledger, and the Coverage Matrix instead of long crossing dataflow lines. Primary dataflow routing prefers ELK layered orthogonal layout and falls back to the built-in deterministic layout when Node.js / elkjs is unavailable. Lines use orthogonal routing and a light halo; ports, protocols, status, source records, relationship types, and source/target objects are shown in collision-aware inline edge detail cards and also kept in the right-side ledger to avoid local label and line stacking.",
        "",
        "Risk and consistency review does not depend on manually reading diagrams. `reports/architecture_findings.md` directly analyzes the graph model, validation findings, risk findings, and reviewable facts generated from the Excel/DCP source, then summarizes services, dependencies, data assets, external systems, network, CI/CD, security, monitoring, IAM, evidence, executive overview readiness, the coverage matrix, missing information and risk list, real dataflow paths, finding groups, and source-sheet fix locations. `reports/architecture_findings.json` provides the same content in machine-readable form.",
        "",
