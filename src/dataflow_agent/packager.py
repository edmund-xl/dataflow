...
def write_package_readme(paths: RuntimePaths, metadata: dict) -> None:
    lines = [
        ...
        *( [f"- 草稿原因：{metadata['draft_reason']}"] if metadata.get("draft_reason") else [] ),
        ...
        "",
        "总览图和服务依赖图采用“入口上下文 + 主数据流 + 控制摘要 + edge ledger”的信息丰富视图：图上只保留真实数据流主线，runtime、Firewall、IAM、Monitoring 作为上下文摘要展示。线条使用分 lane 正交路由、浅色 halo、编号标签和右侧 ledger，便于追溯端口、协议和来源记录。",
        "",
        ...
        *( [f"- Draft reason: {metadata['draft_reason']}"] if metadata.get("draft_reason") else [] ),
        ...
        "",
        "The overview and service dependency diagrams use the information-rich entry-context, primary-dataflow, control-summary, and edge-ledger view: only real primary dataflow lines stay on the canvas, while runtime, Firewall, IAM, and Monitoring relationships stay as context summaries. Lines use lane-based orthogonal routing, a light halo, numbered labels, and the right-side ledger for port, protocol, and source-record traceability.",
        "",
