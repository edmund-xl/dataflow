...
def write_package_readme(paths: RuntimePaths, metadata: dict) -> None:
    lines = [
        ...
        "## 先看这里",
        "",
        "本交付包由 Agent 从数据采集工作簿生成。先看本文件的元数据和结果计数；需要判断缺失信息或风险时，查看 `reports/architecture_findings.md`；需要定位具体修复项时，查看 `reports/validation_report.xlsx` 和 `reports/issue_risk_register.xlsx`。",
        "",
        "## 一、生成元数据",
        "",
        ...
        *( [f"- 草稿原因：{metadata['draft_reason']}"] if metadata.get("draft_reason") else [] ),
        ...
        "## 三、图形产物",
        "",
        "每个分层图同时生成静态图和可编辑源图。SVG、PNG、PDF 用于阅读和归档；Mermaid 用于轻量审查；draw.io `.drawio` 可用 diagrams.net 打开调整展示；GraphML `.graphml` 可导入 yEd、Gephi、Cytoscape。",
        "",
        ...
        "",
        "风险和完整性判断不依赖人工看图。`reports/architecture_findings.md` 直接列出可回溯的缺失信息、风险、真实数据流链路和源表修复位置；`reports/architecture_findings.json` 提供同等内容的机器可读版本。",
        "",
        "draw.io 和 GraphML 只用于展示编辑和工具交换，不是新的事实源。若架构关系、边界或数据流需要变更，必须修正源 Excel/DCP 并重新生成交付包。",
        "",
        "## 四、处理规则",
        "",
        "图、报告和清单均来自同一份结构化工作簿。若产物存在错误，应修正源工作簿并重新运行脚本。",
        "",
        "---",
        "",
        "# English Version",
        ...
        "## Start Here",
        "",
        "This package is generated by the agent from the collection workbook. Read the metadata and result counts first. For missing information or risks, read `reports/architecture_findings.md`. For exact fix locations, read `reports/validation_report.xlsx` and `reports/issue_risk_register.xlsx`.",
        "",
        ...
        *( [f"- Draft reason: {metadata['draft_reason']}"] if metadata.get("draft_reason") else [] ),
        ...
        "## 3. Diagram Artifacts",
        "",
        "Each layered diagram is generated as static output and editable source. SVG, PNG, and PDF are for reading and archiving; Mermaid is for lightweight review; draw.io `.drawio` can be adjusted in diagrams.net; GraphML `.graphml` can be imported into yEd, Gephi, or Cytoscape.",
        "",
        ...
        "",
        "Risk and completeness review does not depend on manually reading diagrams. `reports/architecture_findings.md` lists traceable missing information, risks, real dataflow paths, and source-sheet fix locations. `reports/architecture_findings.json` provides the same content in machine-readable form.",
        "",
        ...
        "## 4. Handling Rule",
        "",
        "The diagrams, reports, and checklists are derived from the same structured workbook. If an artifact is wrong, correct the source workbook and rerun the script.",
        "",
    ]
