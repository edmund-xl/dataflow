...
def write_package_readme(paths: RuntimePaths, metadata: dict) -> None:
    lines = [
        ...
        *( [f"- 草稿原因：{metadata['draft_reason']}"] if metadata.get("draft_reason") else [] ),
        ...
        "",
        "所有分层图采用统一编号拓扑视图：图上线条只显示编号，端口、协议、关系类型和来源记录进入右侧 ledger。总览图和服务依赖图只保留真实主数据流线条；其他图层展示各自范围内的真实关系。没有 graph edge 的关系不会被绘制。",
        "",
        ...
        *( [f"- Draft reason: {metadata['draft_reason']}"] if metadata.get("draft_reason") else [] ),
        ...
        "",
        "All layered diagrams use the unified numbered topology view: line labels are reduced to edge numbers, while ports, protocols, relationship types, and source records move to the right-side ledger. The overview and service dependency diagrams keep only real primary dataflow lines; other layers show the real relationships within their own scopes. Relationships without graph edges are never drawn.",
        "",
