...
def write_package_readme(paths: RuntimePaths, metadata: dict) -> None:
    lines = [
        ...
        *( [f"- 草稿原因：{metadata['draft_reason']}"] if metadata.get("draft_reason") else [] ),
        ...
        "",
        "风险和一致性判断不依赖人工看图。`reports/architecture_findings.md` 会直接分析 Excel/DCP 生成的 graph model、校验结果、风险结果和可审查事实，综合列出服务、依赖、数据资产、外部系统、网络、CI/CD、安全、监控、IAM、证据、真实数据流链路、问题分组和源表修复位置。",
        "",
        ...
        *( [f"- Draft reason: {metadata['draft_reason']}"] if metadata.get("draft_reason") else [] ),
        ...
        "",
        "Risk and consistency review does not depend on manually reading diagrams. `reports/architecture_findings.md` directly analyzes the graph model, validation findings, risk findings, and reviewable facts generated from the Excel/DCP source, then summarizes services, dependencies, data assets, external systems, network, CI/CD, security, monitoring, IAM, evidence, real dataflow paths, finding groups, and source-sheet fix locations.",
        "",
