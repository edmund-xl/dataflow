...
def write_package_readme(paths: RuntimePaths, metadata: dict) -> None:
    lines = [
        ...
        *( [f"- 草稿原因：{metadata['draft_reason']}"] if metadata.get("draft_reason") else [] ),
        ...
        "",
        "总览图和服务依赖图采用“入口上下文 + 主数据流 + 控制摘要 + edge ledger”的信息丰富视图：图上只保留真实数据流主线，runtime、Firewall、IAM、Monitoring 作为上下文摘要展示。线条使用分 lane 正交路由、浅色 halo 和紧凑编号标签；端口、协议、状态和来源记录统一放在右侧 ledger，避免长标签覆盖线条。",
        "",
        ...
        *( [f"- Draft reason: {metadata['draft_reason']}"] if metadata.get("draft_reason") else [] ),
        ...
        "",
        "The overview and service dependency diagrams use the information-rich entry-context, primary-dataflow, control-summary, and edge-ledger view: only real primary dataflow lines stay on the canvas, while runtime, Firewall, IAM, and Monitoring relationships stay as context summaries. Lines use lane-based orthogonal routing, a light halo, and compact numbered labels; ports, protocols, status, and source records are kept in the right-side ledger to avoid long labels covering the lines.",
        "",
