...
先阅读 `check_summary.md`。如果状态为 `NEEDS_FIX`，继续阅读 `fix_list.md`，修正源工作簿后再次执行脚本。需要从内容本身判断架构、安全、监控和证据问题时，阅读 `architecture_findings.md`；该报告直接分析 Excel/DCP 生成的 graph model，不依赖人工看图，并区分自动化问题和需要人工复核的审查观察项。
