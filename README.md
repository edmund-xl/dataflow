...
分层图采用统一的内置编号拓扑渲染器生成：每张正式图都使用相同视觉风格，图上线条只显示编号，右侧 `Edge ledger` 记录来源工作簿记录、关系类型、端口和源/目标对象。渲染不依赖外部图形命令，能够稳定生成 SVG、PNG、PDF、Mermaid 调试文件、draw.io 可编辑源图和 GraphML 工具交换文件。draw.io / GraphML 只用于展示和二次编辑，架构事实仍以源工作簿为准；如果关系需要变更，应修改 Excel/DCP 后重新生成。

总览图和服务依赖图只展示真实主数据流关系；网络、计算服务、数据存储、安全监控、CI/CD 图则展示各自图层内的真实关系。所有图层都遵守同一规则：没有 graph edge 的关系绝不补画，长标签进入 ledger，避免压在线路上导致不可读。
...
Layered diagrams are generated by the unified built-in numbered topology renderer: every formal diagram uses the same visual style, line labels are reduced to edge numbers, and the right-side `Edge ledger` records the source workbook row, relationship type, port, source object, and target object. Rendering does not depend on external graph commands and consistently produces SVG, PNG, PDF, Mermaid debug files, draw.io editable source files, and GraphML exchange files. draw.io and GraphML outputs are for presentation editing and tool import; the source workbook remains the factual source of truth. If a relationship changes, update the Excel/DCP and regenerate the package.

The overview and service dependency layers show only real primary dataflow relationships; the network, compute/service, data/storage, security/monitoring, and CI/CD layers show the real relationships within their own layer scopes. Every layer follows the same rule: relationships without graph edges are never drawn, and long labels move to the ledger instead of sitting on top of lines.
