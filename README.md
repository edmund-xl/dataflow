...
分层图采用内置专业渲染器生成：常规视图使用 C4 架构图风格，安全与监控视图使用深色审计风格。渲染不依赖外部图形命令，能够稳定生成 SVG、PNG、PDF 和 Mermaid 调试文件。
...
```bash
scripts/check_dcp.sh samples/DCP_v0.1 --output /tmp/agent_check
```
...
```bash
scripts/build_dataflow_package.sh samples/DCP_v0.1 --output /tmp/dataflow_dist
```
...
当前可直接运行的 Agent 样例位于 `samples/DCP_v0.1/`。该样例工作簿已经预留 runtime 字段和显式依赖目标字段，包括 `Runtime_Type`、`Runtime_ID`、`Runtime_Name`、`Runtime_Namespace`、`Runtime_Cluster`、`Runtime_Region`、`Target_Type`、`Target_ID` 和 `Interaction_Mode`。
...
Layered diagrams are generated by the built-in professional renderer: regular views use a C4 architecture style, and the security and monitoring view uses a dark audit style. Rendering does not depend on external graph commands and consistently produces SVG, PNG, PDF, and Mermaid debug files.

## 4. Script-First Workflow

### Environment Doctor

Before first use or after moving to another machine, run:
...
The directly runnable agent sample is under `samples/DCP_v0.1/`. Its workbook now exposes runtime fields and explicit dependency target fields, including `Runtime_Type`, `Runtime_ID`, `Runtime_Name`, `Runtime_Namespace`, `Runtime_Cluster`, `Runtime_Region`, `Target_Type`, `Target_ID`, and `Interaction_Mode`.
